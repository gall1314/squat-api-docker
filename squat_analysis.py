...
# ================== THEME (אחיד לכל התרגילים) ==================
FONT_PATH = "Roboto-VariableFont_wdth,wght.ttf"

# ברים / טיפוגרפיה
TOP_BAR_FRAC      = 0.095   # פס עליון גדול יותר כדי לכסות גם Reps וגם הדונאט
BOTTOM_BAR_FRAC   = 0.08
BAR_BG_OPACITY    = 0.58    # כהה מעט יותר לחיתוך נקי

REPS_FONT_PX      = 30
DEPTH_LABEL_PX    = 16
DEPTH_PCT_PX      = 22
FEEDBACK_BASE_PX  = 26      # יוקטן אוטומטית אם אין מקום
FEEDBACK_MIN_PX   = 18
PADDING           = 14

# דונאט
DEPTH_RADIUS_SCALE   = 0.78  # יחס לרוחב/גובה הפס העליון
DEPTH_THICKNESS_FRAC = 0.28
DEPTH_COLOR          = (40, 200, 80)   # BGR
DEPTH_RING_BG        = (70, 70, 70)    # BGR

# דינמיקת דונאט (סמוזינג + Peak Hold)
PERFECT_MIN_KNEE     = 70
...
# ספירת חזרות – היסטרזיס/דיבאונס
KNEE_DOWN_TRIG     = 140   # כניסה ל-down (רך יותר)
KNEE_BOTTOM_ZONE   = 95    # אזור "תחתית"
KNEE_UP_TRIG       = 165   # נעילה למעלה
MIN_FRAMES_DOWN    = 3     # חייבים לפחות X פריימים ב-down
MIN_FRAMES_AT_TOP  = 2     # חייבים X פריימים למעלה כדי לאשר סיום חזרה
...
def draw_overlay(frame, reps=0, feedback=None, depth_pct=0.0):
    ...
    # דונאט מימין – בתוך הפס העליון (יותר נמוך כדי שלא ייחתך)
    radius = int(top_h * DEPTH_RADIUS_SCALE / 2)
    thick  = max(3, int(radius * DEPTH_THICKNESS_FRAC))
    cx = w - PADDING - radius
    cy = int(top_h * 0.60)  # טיפונת נמוך יותר
    frame = np.array(pil)
    ...
    # פס תחתון לפידבק – עם התאמת גודל דינמית כדי שלא ייחתך
    if feedback:
        over = frame.copy()
        ...
        draw = ImageDraw.Draw(pil)
        max_w = w - 2*PADDING
        # אם ארוך – נפריד עם " | " ונכווץ פונט עד שזה נכנס בשורה אחת
        fb_text = feedback
    ...
    with mp_pose.Pose(
        ...
        while cap.isOpened():
            ...
            try:
                ...
                pts = lmk_smoother.apply(lm, w, h)  # החלקה נגד קפיצות
                frame = draw_body_skeleton(frame, pts)

                # שימוש ב-right side כבעבר
                ...
                # לחשב זוויות עם utils (קואורדינאטות נורמליזציה כבעבר)
                ...
                if stage == "down":
                    ...
                    frames_top += 1  # דרוש כדי לאשר שעמדנו למעלה מספיק זמן (אם תרצה להשתמש בו)
