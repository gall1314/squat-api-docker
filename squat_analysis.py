# -*- coding: utf-8 -*-
# squat_analysis.py
# בסיס שסופר טוב + Overlay צמוד לפינה
# פאי-גרף "לייב" ללא דיליי (עולה/יורד בזמן אמת),
# פידבק גב מדויק, ו-RT feedback עם Hold.
...
def merge_feedback(global_best, new_list):
    ...
    return (
        cand
        if FB_SEVERITY.get(cand, 1) >= FB_SEVERITY.get(global_best, 1)
        else global_best
    )
...
def run_squat_analysis(
    ...
    with mp_pose.Pose(
        ...
        while cap.isOpened():
            ...
            if out is None:
                out = cv2.VideoWriter(
                    output_path, fourcc, effective_fps, (w, h)
                )
            ...
            shoulder = np.array(
                [lm[R.RIGHT_SHOULDER.value].x, lm[R.RIGHT_SHOULDER.value].y]
            )
            ...
            if knee_angle > STAND_KNEE_ANGLE - 5:
                stand_knee_ema = (
                    knee_angle
                    if stand_knee_ema is None
                    else STAND_KNEE_ALPHA * knee_angle
                    + (1 - STAND_KNEE_ALPHA) * stand_knee_ema
                )

            if stand_knee_ema is not None:
                denom = max(10.0, stand_knee_ema - PERFECT_MIN_KNEE_SQ)
                depth_live = np.clip(
                    (stand_knee_ema - knee_angle) / denom, 0, 1
                )
                ...
                rep_reports.append(
                    {
                        ...
                        "feedback": (
                            [pick_strongest_feedback(feedbacks)]
                            if feedbacks
                            else []
                        ),
