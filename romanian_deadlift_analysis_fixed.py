...
# ✅ FIX 1: Video rotation handling for mobile videos
# ✅ FIX 2: Full-resolution overlay (matches good_morning approach)
# ✅ FIX 3: Skeleton drawing at original resolution
# ✅ FIX 4: Person locking to prevent skeleton jumping to another person
    ...
    with mp_pose_mod.Pose(model_complexity=model_complexity,
        ...
        while cap.isOpened():
            ...
            if not person_locker.check(lm):
                # Rejected - skeleton jumped to another person
                if create_video and out is not None:
                    frame_out = cv2.resize(work, (out_w, out_h))
                    if last_valid_lm is not None:
                        frame_out = draw_body_only(frame_out, last_valid_lm)
