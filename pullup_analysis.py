# -*- coding: utf-8 -*-
# pullup_analysis.py — ספירה מתוקנת (נעילת צד), זרימה כמו סקוואט, אוברליי תואם,
# ולוגים שמראים למה נכנס/לא נכנס לסט. שם החזרה: squat_count.
...
class PullUpAnalyzer:
    ...
    def reset(self):
        ...
        self.side_locked=None  # <<< נעילת צד לסשן
    ...
    def process(self, input_path, output_path=None, frame_skip=3, scale=0.4, overlay_enabled=True):
        ...
        self.RT_FB_HOLD_FRAMES=max(2,int(0.8/(1.0/effective_fps)))

        print(f"[PULLUP] open | fps_in={fps_in:.2f} | frame_skip={frame_skip} | eff_fps={effective_fps:.2f}", flush=True)

        out=None; frame_idx=0; rep_baseline_head_y=None; last_log=time.time()
            ...
            while True:
                ...
                if not results.pose_landmarks:
                    ...
                    if time.time()-last_log>0.3:
                        print(f"[PULLUP] f{frame_idx} reps={self.rep_count} (no landmarks)", flush=True)
                        last_log=time.time()
                    continue

                lms=results.pose_landmarks.landmark
                if self.side_locked is None:
                    self.side_locked=_pick_side(lms)  # <<< נועלים צד פעם אחת
                    ...
                    if not self.in_rep:
                        if self._confirm_start(elbow_angle, head_vel):
                            ...
                            if time.time()-last_log>0.6:
                                print(f"[PULLUP] waiting start | elbow={elbow_angle:.1f} (thr<{ELBOW_START_THRESHOLD}) head_vel={head_vel:.4f} (need<-{HEAD_VEL_UP_THRESH})", flush=True)
                                last_log=time.time()
                    else:
...
def run_pullup_analysis(input_path, frame_skip=3, scale=0.4, output_path=None, overlay_enabled=True):
    ...
    t0=time.time(); print(f"[PULLUP] start | input={input_path} | skip={frame_skip} | scale={scale}", flush=True)
