# -*- coding: utf-8 -*-
# pullup_analysis.py — “מקורי” + תיקוני פספוס (rearm רך + post-hoc)
...
def run_pullup_analysis(video_path,
    ...
    # fast_mode: לא נוגעים ב-frame_skip (נשאר 3 כמו במקור) — רק מכניסים model_complexity=0 ובלי וידאו
            ...
            if onbar and vis_strict_ok:
                if asc_base_head is None:
                    ...
                    if reset_by_desc or reset_by_elb:
                        ...
                        asc_base_head=head_y; rep_penalty_current=0.0
                        cycle_peak_ascent=0.0; cycle_min_elbow=elbow_angle; counted_this_cycle=False
                        allow_new_peak=True; bottom_phase_max_elbow=None; bottom_already_reported=False
                ...
                if (allow_new_peak is False) and (last_peak_frame>0):
                    if (head_y - res.pose_landmarks.landmark[NOSE].y + 0.0) or True:  # noop; placeholder
                        pass
                    # Rearm על דיפ קטן מהטופ (לפתור #9)
                    if (asc_base_head is not None) and ((head_y - (head_y if last_peak_frame==frame_idx else head_y)) >= 0):
                        pass
                    # Rearm פשוט לפי ירידה מהטופ:
