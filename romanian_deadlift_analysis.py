...
def analyze_back_curvature(shoulder, hip, head_like, max_angle_deg=45.0, min_head_dist_ratio=0.35):
    tv = shoulder - hip; hv = head_like - shoulder
    tn = np.linalg.norm(tv) + 1e-9; hn = np.linalg.norm(hv) + 1e-9
...
def _get_all_landmarks(lm):
    ...
    left  = {"shoulder": _pt(PL.LEFT_SHOULDER),  "hip": _pt(PL.LEFT_HIP),
             "knee": _pt(PL.LEFT_KNEE), "ankle": _pt(PL.LEFT_ANKLE), "ear": _pt(PL.LEFT_EAR)}
    right = {"shoulder": _pt(PL.RIGHT_SHOULDER), "hip": _pt(PL.RIGHT_HIP),
             "knee": _pt(PL.RIGHT_KNEE), "ankle": _pt(PL.RIGHT_ANKLE), "ear": _pt(PL.RIGHT_EAR)}
    ...
    return {"left": left, "right": right, "dominant": left if lv >= rv else right,
            "dominant_vis": max(lv, rv), "mid": mid, "left_vis": lv, "right_vis": rv}
...
def _get_side_landmarks(lm):
    ...
    return {k: np.array([lm[idx[i].value].x, lm[idx[i].value].y]) for i, k in enumerate(keys)}

def compute_movement_signal(all_lm):
    mid = all_lm["mid"]; dom = all_lm["dominant"]
    hy, sy, ay = mid["hip"][1], mid["shoulder"][1], mid["ankle"][1]
    bh = abs(ay - sy) + 1e-6
    sd = float(np.clip(((sy - hy) / bh + 0.4) / 0.5, 0.0, 1.0))
    sn = float(np.clip((sy - hy + 0.3) / 0.4, 0.0, 1.0))
    sh2, hi2, kn2, an2 = dom["shoulder"][:2], dom["hip"][:2], dom["knee"][:2], dom["ankle"][:2]
    ...
    t3a = math.degrees(math.acos(float(np.clip(np.dot(t3, np.array([0.0, -1.0, 0.0])) / t3n, -1.0, 1.0))))
    t3s = float(np.clip(t3a / 80.0, 0.0, 1.0))
    vr = min(all_lm["left_vis"], all_lm["right_vis"]) / (max(all_lm["left_vis"], all_lm["right_vis"]) + 1e-6)
    if   vr > 0.7: w = {"sd": .30, "sn": .20, "ts": .05, "t3": .30, "ks": .15}
    elif vr > 0.4: w = {"sd": .20, "sn": .15, "ts": .20, "t3": .25, "ks": .20}
    else:          w = {"sd": .10, "sn": .10, "ts": .40, "t3": .20, "ks": .20}
    c = float(np.clip(w["sd"]*sd + w["sn"]*sn + w["ts"]*ts + w["t3"]*t3s + w["ks"]*ks, 0.0, 1.0))
    return {"composite": c, "shoulder_drop": sd, "shoulder_y": sn,
            "torso_2d": ts, "torso_3d": t3s, "torso_2d_angle": ta, "torso_3d_angle": t3a,
            "knee_signal": ks, "knee_angle": ka, "vis_ratio": vr, "weights": w}

class DepthNormalizer:
    def __init__(self):
        self.floor = 0.50; self.ceiling = 0.50; self.n = 0
    def update(self, raw):
        self.n += 1
        if raw < self.floor: self.floor = 0.85 * self.floor + 0.15 * raw
        else:                self.floor = 0.995 * self.floor + 0.005 * raw
        if raw > self.ceiling: self.ceiling = 0.80 * self.ceiling + 0.20 * raw
        else:                  self.ceiling = 0.995 * self.ceiling + 0.005 * raw
        rng = max(0.12, self.ceiling - self.floor)
        return float(np.clip((raw - self.floor) / rng, 0.0, 1.0))

class RepCounter:
    ENTER_THRESHOLD = 0.30; EXIT_THRESHOLD = 0.18; MIN_PEAK_FOR_REP = 0.45
    GOOD_DEPTH_PEAK = 0.58; MIN_FRAMES_BETWEEN = 8; MAX_REP_FRAMES = 120
    REBOUND_DELTA = 0.06; MIN_RETURN_FROM_PEAK = 0.10
    NORMALIZED_PEAK_THRESHOLD = 0.62; NORMALIZED_DROP_THRESHOLD = 0.18
    MIN_REP_DURATION_FRAMES = 10; MIN_PEAK_DELTA = 0.12; MIN_NORMALIZED_PEAK_DELTA = 0.25

    def __init__(self):
        self.state = "standing"; self.count = 0; self.current_peak = 0.0
        self.frames_in_state = 0; self.last_rep_frame = -999; self.smoothed = 0.0
        self.rep_start_frame = -1; self.ascent_valley = 1.0; self.rep_start_signal = 0.0
        self.rep_max_torso_2d = 0.0; self.rep_max_torso_3d = 0.0
        self.rep_min_knee = 999.0; self.rep_max_knee = 0.0
        self.rep_back_issue = False; self.rep_back_angle = 0.0
        self.calibration_signals = []; self.calibrated = False
        self.standing_baseline = 0.0; self.dynamic_floor = 0.0; self.dynamic_ceil = 0.6

    def _smooth(self, raw):
        self.smoothed += 0.35 * (raw - self.smoothed); return self.smoothed
    def _normalize(self, s):
        ...
        return float(np.clip((s - self.dynamic_floor) / max(0.20, self.dynamic_ceil - self.dynamic_floor), 0.0, 1.0))
    def _calibrate(self, signal):
        ...
        if len(self.calibration_signals) >= 25:
            ...
            self.dynamic_ceil = max(self.standing_baseline + 0.40, float(np.percentile(self.calibration_signals, 95)))
    ...
    def _start_rep(self, sd, fi, sm):
        self.state = "descending"; self.current_peak = sm; self.frames_in_state = 0
        self.rep_start_frame = fi; self.ascent_valley = sm; self.rep_start_signal = sm
        self.rep_max_torso_2d = sd.get("torso_2d_angle", 0)
        self.rep_max_torso_3d = sd.get("torso_3d_angle", 0)
        self.rep_min_knee = sd.get("knee_angle", 170); self.rep_max_knee = sd.get("knee_angle", 170)
        self.rep_back_issue = False; self.rep_back_angle = 0.0
    def _norm_peak(self):
    ...
    def _rep_info(self):
        return {"rep": self.count, "peak_signal": self.current_peak,
                "good_depth": self.current_peak >= self.GOOD_DEPTH_PEAK,
                "max_torso_2d": self.rep_max_torso_2d, "max_torso_3d": self.rep_max_torso_3d,
                "min_knee": self.rep_min_knee, "max_knee": self.rep_max_knee,
                "back_issue": self.rep_back_issue, "back_angle": self.rep_back_angle}
    def _reset(self):
        self.state = "standing"; self.frames_in_state = 0; self.current_peak = 0.0
        self.rep_start_frame = -1; self.ascent_valley = 1.0; self.rep_start_signal = 0.0

    def finalize_pending_rep(self, fi):
        if self.state != "ascending": return None
        ...
        ok_r = vd >= self.MIN_RETURN_FROM_PEAK * 0.7 or nvd >= self.NORMALIZED_DROP_THRESHOLD * 0.8
        if (ok_p and ok_r and self._meaningful()
            ...
            self.count += 1; self.last_rep_frame = fi
            info = self._rep_info(); self._reset(); return info
        return None

    def update(self, sd, fi):
        raw = sd["composite"]; self._calibrate(raw)
        sm = self._smooth(raw); nm = self._normalize(sm)
        ...
        if self.state in ("descending", "ascending"):
            ...
            self.rep_min_knee = min(self.rep_min_knee, ka); self.rep_max_knee = max(self.rep_max_knee, ka)
        if self.state == "standing":
            if sm >= self.ENTER_THRESHOLD or nm >= 0.34:
                self._start_rep(sd, fi, sm)
        elif self.state == "descending":
            if sm > self.current_peak: self.current_peak = sm
            if sm < self.current_peak - 0.015 and self.frames_in_state >= 2:
                self.state = "ascending"; self.frames_in_state = 0; self.ascent_valley = sm
            if self.rep_start_frame > 0 and (fi - self.rep_start_frame) > self.MAX_REP_FRAMES:
                self._reset()
        elif self.state == "ascending":
            self.ascent_valley = min(self.ascent_valley, sm)
            if (sm <= self.EXIT_THRESHOLD or nm <= 0.28) and (fi - self.last_rep_frame) >= self.MIN_FRAMES_BETWEEN:
                if ((self.current_peak >= self.MIN_PEAK_FOR_REP or self._norm_peak() >= self.NORMALIZED_PEAK_THRESHOLD)
                        and self._meaningful() and (fi - self.rep_start_frame) >= self.MIN_REP_DURATION_FRAMES):
                    self.count += 1; self.last_rep_frame = fi
                    info = self._rep_info(); self._reset(); return info
                self._reset()
            vd = self.current_peak - self.ascent_valley
            ...
            if sm > (self.ascent_valley + self.REBOUND_DELTA) and self.frames_in_state >= 2:
                ...
                if (ok_p and ok_r and self._meaningful()
                        ...
                        and (fi - self.rep_start_frame) >= self.MIN_REP_DURATION_FRAMES):
                    self.count += 1; self.last_rep_frame = fi
                    info = self._rep_info(); self._start_rep(sd, fi, sm); return info
            if sm > self.current_peak:
                self.current_peak = sm; self.state = "descending"; self.frames_in_state = 0
            if self.rep_start_frame > 0 and (fi - self.rep_start_frame) > self.MAX_REP_FRAMES:
                self._reset()
        return None

HINGE_BOTTOM_ANGLE = 55.0; KNEE_MIN_ANGLE = 172.0; KNEE_MAX_ANGLE = 125.0
BACK_MAX_ANGLE = 60.0; MIN_SCORE = 4.0; MAX_SCORE = 10.0
...
def _get_rotation_angle(video_path):
    ...
    try:
        import json
        r = subprocess.run(["ffprobe", "-v", "quiet", "-show_streams", "-of", "json", video_path],
            capture_output=True, text=True, timeout=10)
        for st in json.loads(r.stdout).get("streams", []):
            if st.get("codec_type") == "video":
                if "rotate" in st.get("tags", {}): return int(st["tags"]["rotate"])
                for sd in st.get("side_data_list", []):
                    if "rotation" in sd:
                        a = int(float(sd["rotation"])); return 360 + a if a < 0 else a
    except Exception: pass
    return 0
...
def _evaluate_rep(rep_result):
    ...
    if not rep_result["good_depth"] and best_t < HINGE_BOTTOM_ANGLE:
        feedback.append("Go deeper - hinge more at the hips"); score -= 2.0
    if rep_result["max_knee"] > KNEE_MIN_ANGLE:
        feedback.append("Bend your knees a bit more"); score -= 1.5
    elif rep_result["min_knee"] < KNEE_MAX_ANGLE:
        feedback.append("Too much knee bend"); score -= 2.0
    if rep_result["back_issue"] and rep_result["back_angle"] > (BACK_MAX_ANGLE + 5.0):
        feedback.append("Try to keep your back neutral"); score -= 1.0
...
def _snapshot_lm(lm):
    ...
    class _P:
        ...
        def __init__(self, src):
            self.x = src.x; self.y = src.y; self.z = src.z; self.visibility = src.visibility
...
def _analysis_pass(video_path, rotation, frame_skip, scale, fps_in):
    ...
    rep_counter = RepCounter(); depth_norm = DepthNormalizer()
    good_reps = bad_reps = 0
    all_scores, rep_reports, session_feedbacks = [], [], []
    session_feedback_by_cat = {}
    frame_idx = 0; depth_pct = 0.0; rt_fb_msg = None; rt_fb_hold = 0
    frame_data = {}; last_valid_lm = None
    cap = cv2.VideoCapture(video_path); t0 = time.time()
    ...
    with mp_pose.Pose(model_complexity=0, min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        ...
        while cap.isOpened():
            ...
            if not ret: break
            if time.time() - t0 > 180:
                print("[RDL] Pass1 timeout", file=sys.stderr, flush=True); break
            frame_idx += 1
            if frame_idx % effective_skip != 0: continue
            ...
            def _store(snap):
                frame_data[frame_idx] = {"lm": snap, "reps": rep_counter.count,
                    "fb": rt_fb_msg if rt_fb_hold > 0 else None, "prog": depth_pct}
            ...
            snap = _snapshot_lm(lm); last_valid_lm = snap
            ...
            back_angle, back_bad = analyze_back_curvature(pts["shoulder"], pts["hip"], pts["ear"], BACK_MAX_ANGLE)
            ...
            if rep_result is not None:
                ...
                rep_reports.append({
                    "rep": rep_result["rep"], "score": round(score, 2),
                    "score_display": display_half_str(score), "label": score_label(score),
                    "feedback": feedback,
                    "metrics": {"peak_signal": round(rep_result["peak_signal"], 3),
                        ...
                        "back_angle": round(rep_result["back_angle"], 2), "view_type": vt}})
                print(f"[RDL] Rep {rep_result['rep']} score={score:.1f} peak={rep_result['peak_signal']:.3f}",
    ...
    if rep_result is not None:
        ...
        session_feedbacks.extend(feedback)
        _, session_feedback_by_cat = pick_strongest_per_category(session_feedbacks)
        rep_reports.append({"rep": rep_result["rep"], "score": round(score, 2),
            "score_display": display_half_str(score), "label": score_label(score),
            "feedback": feedback, "metrics": {"peak_signal": round(rep_result["peak_signal"], 3),
                "max_torso_2d_angle": round(rep_result["max_torso_2d"], 2),
                "max_torso_3d_angle": round(rep_result["max_torso_3d"], 2),
                "min_knee_angle": round(rep_result["min_knee"], 2),
                "max_knee_angle": round(rep_result["max_knee"], 2),
                "back_angle": round(rep_result["back_angle"], 2), "view_type": "end_of_clip"}})
    ...
    feedback_list = (dedupe_feedback(list(session_feedback_by_cat.values()))
        if session_feedback_by_cat
        else (["Perfect form! \U0001f525"] if not session_feedbacks else dedupe_feedback(session_feedbacks)))
    ...
    # positive_feedback: True when score=10 and feedback is the positive message
    is_positive = (technique_score >= 10.0 and counter > 0 and not session_feedbacks)

    return {
        "squat_count": int(counter), "technique_score": float(technique_score),
        ...
        "feedback": [str(f) for f in feedback_list], "tip": str(tip),
        "reps": rep_reports, "positive_feedback": bool(is_positive),
...
def _render_pass(video_path, rotation, frame_skip, output_path, out_w, out_h, fps_in, frame_data):
    ...
    while cap.isOpened():
        ...
        if fd is None:
            ...
            f = frame.copy()
            if fd["lm"] is not None: f = draw_body_only(f, fd["lm"])
            out.write(draw_overlay(f, reps=fd["reps"], feedback=fd["fb"], depth_pct=fd["prog"]))
        written += 1
...
# PUBLIC ENTRY POINT
# =====================================================================
def run_romanian_deadlift_analysis(video_path, frame_skip=3, scale=0.4,
                                   output_path="romanian_deadlift_analyzed.mp4",
                                   feedback_path="romanian_deadlift_feedback.txt",
                                   return_video=True, fast_mode=False):
    import sys
    print(f"[RDL] Start fast_mode={fast_mode} return_video={return_video}", file=sys.stderr, flush=True)
    if fast_mode: return_video = False
    ...
    cap = cv2.VideoCapture(video_path)
    if not cap.isOpened():
        return {"squat_count": 0, "technique_score": 0.0, "technique_score_display": "0",
                "technique_label": score_label(0), "good_reps": 0, "bad_reps": 0,
                "feedback": ["Could not open video"], "reps": [], "video_path": "",
                "feedback_path": feedback_path, "positive_feedback": False}
    ...
    if total_frames < 10:
        return {"squat_count": 0, "technique_score": 0.0, "technique_score_display": "0",
                ...
                "reps": [], "video_path": "", "feedback_path": feedback_path, "positive_feedback": False}
    ...
    try:
        with open(feedback_path, "w", encoding="utf-8") as f:
            f.write(f"Total Reps: {analysis['squat_count']}\nTechnique Score: {analysis['technique_score']}/10\n")
            for fb in analysis["feedback"]: f.write(f"- {fb}\n")
    ...
    if create_video:
        ...
        try:
            ...
            if proc.returncode != 0:
                print(f"[RDL] ffmpeg stderr: {proc.stderr.decode(errors='replace')[-500:]}", file=sys.stderr, flush=True)
        except Exception as e:
