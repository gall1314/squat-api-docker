# -*- coding: utf-8 -*-
# barbell_bicep_curl.py — זהות מלאה לסקוואט בסטייל/וידאו/קידוד/Overlay/פונטים/דילוג פריימים.
import os
import cv2
import math
import numpy as np
...
# ========= נסה לייבא בדיוק מהסקוואט כדי להבטיח תאימות פרפקט =========
_IMPORTED = False
try:
    from squat_analysis import (
        draw_overlay, draw_body_only, display_half_str, score_label,
        REPS_FONT_SIZE, FEEDBACK_FONT_SIZE, DEPTH_LABEL_FONT_SIZE, DEPTH_PCT_FONT_SIZE,
        DONUT_RADIUS_SCALE, DONUT_THICKNESS_FRAC, BAR_BG_ALPHA
    )
    _IMPORTED = True
except Exception:
    _IMPORTED = False

# ========= Fallback 1:1 מסקוואט (אם הייבוא לא קיים) =========
if not _IMPORTED:
    # ---- STYLE / FONTS ----
    BAR_BG_ALPHA         = 0.55
...
# ========= פרמטרים — זהים לקצב/דיליי מסקוואט =========
...
def run_barbell_bicep_curl_analysis(video_path,
    ...
    with mp_pose_mod.Pose(model_complexity=1, min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while cap.isOpened():
            ...
            # ללא לנדמארקס — עדיין מצייר Overlay זהה
            ...
            try:
                ...
                # מהירויות גלובליות (סינון הליכה/תנועה לא קשורה) — זהה
                ...
                # עומק "דונאט" — 1:1 לסקוואט (הנוסחה מותאמת לקרל)
                ...
                # נגזרת זווית (סיווג שלב)
                deriv = None
                if last_angle is not None:
                    deriv = elbow_angle - last_angle  # >0 ירידה (אקסצנטרי), <0 עליה
                last_angle = elbow_angle

                # התחלת רפ — מהתחתית, כשמתחילים לעלות (פלקס)
                ...
                if stage in ("up","down"):
                    ...
                    # RT-feedback כמו סקוואט (עם hold)
                    if stage == "up":
                ...
                # סיום רפ — חזרה ליישור + שקט תנועתי (זהה)
                if (stage in ("up","down")) and (elbow_angle >= ELBOW_EXT_END_THR) and (movement_free_streak >= MOVEMENT_CLEAR_FRAMES):
                    if frame_idx - (rep_start_frame or frame_idx) > 3:
                        ...
                        rep_reports.append({
                            ...
                            "feedback": (["Try to curl higher — aim to squeeze at the top"] if (rep_min_elbow_angle > MAX_TOP_FLEXION_ANGLE) else [] ) +
                                        (["Straighten your arms fully at the bottom for a full range of motion"] if (rep_max_elbow_angle < MIN_BOTTOM_EXTENSION_ANGLE) else []),
                            "tip": tip_msg,
                        ...
                        # פידבק-סשן — strongest only (כמו בסקוואט)
                        sess_cand = []
                        if rep_min_elbow_angle > MAX_TOP_FLEXION_ANGLE: sess_cand.append("Try to curl higher — aim to squeeze at the top")
                        if rep_max_elbow_angle < MIN_BOTTOM_EXTENSION_ANGLE: sess_cand.append("Straighten your arms fully at the bottom for a full range of motion")
                        # (דריפט מרפקים אפשר להוסיף מאוחר יותר)
    ...
    # קידוד faststart זהה (yuv420p, preset=fast)
    ...
    return {
        "squat_count": counter,                                  # נשמר לשמירה על תאימות UI
